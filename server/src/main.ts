...
let matchJoin: nkruntime.MatchJoinFunction = function (
  ...
  if (gameState.stage === "inProgress") {
    ...
    if (timeLeft > 1000) {
      for (const presence of presences) {
        if (gameState.playerToResult[presence.userId]) {
          ...
          dispatcher.broadcastMessage(
            OpCode.NEXT_STEP,
            encodeMessageData({ step: gameState.currentStep, last: gameState.lastStep, timeout: timeLeft, lines: gameState.lastLinesByPlayer[presence.userId], input, active: true }),
            [presence]
          );
        } else {
          // spectator
      ...
      dispatcher.broadcastMessage(
        OpCode.READY_UPDATE,
        encodeMessageData({ ready: Object.keys(gameState.playersReadyForNextStep).length, total: Object.keys(gameState.gameResults).length }),
        presences
      );
    }
  ...
  for (const message of messages) {
    if (message.opCode === OpCode.KICK_PLAYER) {
      ...
      for (const userId of Object.keys(gameState.presences)) {
        dispatcher.broadcastMessage(
          OpCode.NEXT_STEP,
          encodeMessageData({ step: gameState.currentStep, last: gameState.lastStep, timeout: zeroStepDuration, active: Boolean(gameState.playerToResult[userId]) }),
          [gameState.presences[userId]]
        );
      }
      ...
      if (gameState.currentStep === gameState.lastStep) {
        ...
        for (const userId of Object.keys(gameState.presences)) {
          if (!gameState.playerToResult[userId]) {
            dispatcher.broadcastMessage(OpCode.NEXT_STEP, encodeMessageData({ step: gameState.currentStep, last: gameState.lastStep, timeout: gameState.settings.stepDuration, active: false }), [
              gameState.presences[userId],
            ]);
            continue;
          }
          ...
          const lines = gameState.gameResults[resultId]
            ...
            .map((line, i, inputs) => {
              ...
              const hiddenLetters = line.input.replace(/[a-zа-яё]/giu, "∗");

              if (!gameState.settings.revealLastWordInLines) {
                return hiddenLetters;
              }

              const letters = line.input.match(/[a-zа-яё]/giu);

              if (!letters) {
                return hiddenLetters;
              }

              const lastWordMatch = line.input.match(/.*(^|[^a-zа-яё])([a-zа-яё]+)/iu);
          ...
          dispatcher.broadcastMessage(OpCode.NEXT_STEP, encodeMessageData({ step: gameState.currentStep, last: gameState.lastStep, timeout: gameState.settings.stepDuration, lines, active: true }), [
            gameState.presences[userId],
          ]);
        }
      }
